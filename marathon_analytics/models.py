...
class Result(models.Model):
    ...
    def get_runners_passed(self):
        ...
        print(started_first[:5])
        # passed = started_first.filter(finish_time_of_day__gt=self.finish_time_of_day)
        # return len(passed)
        return Result.objects.filter(
            start_time_of_day__lt=self.start_time_of_day,
            finish_time_of_day__gt=self.finish_time_of_day
        ).count()
    ...
    def get_runners_passed_by(self):
        ...
        return Result.objects.filter(
            start_time_of_day__gt=self.start_time_of_day,
            finish_time_of_day__lt=self.finish_time_of_day
        ).count()
...
def load_data():
    ...
    for line in f:
        ...
        try:
            ...
            result = Result(bib=fields[0], first_name=fields[1], last_name=fields[2],
            ctz = fields[3], city = fields[4], state = fields[5], gender = fields[6],
            division = fields[7], place_overall = fields[8], place_gender = fields[9],
            place_division = fields[10], start_time_of_day = fields[11],
            finish_time_of_day = fields[12],time_finish = fields[13],
            time_half1 = fields[14], time_half2 = fields[15], )
